    ...
    foreach($result AS $targetKey => $targetValue) {
      foreach($targetValue AS $k => $dataset) {
        if($time >= $dataset['begin'] && $time <= $dataset['end']) {
          if(!isset($result[$targetKey][$k]['count'][$station])) {
            $result[$targetKey][$k]['count'][$station] = 0;
            $result[$targetKey][$k]['sum'][$station] = 0;
          }
          ++$result[$targetKey][$k]['count'][$station];
          $result[$targetKey][$k]['sum'][$station] += $data[$targets[$targetKey]];
        }
      }
    }
  }
...
foreach($result AS $targetKey => $targetValue) {
  ...
  foreach($targetValue AS $k => $dataset) {
    foreach($dataset['count'] AS $cityKey => $val) {
      $parts = explode('-', $cityKey);
      if(!isset($datasets[$targetKey][$cityKey])) {
        $datasets[$targetKey][$cityKey] = array(
          'label' => $cityKey,
          'backgroundColor' => $cityColor[$parts[0]],
          'borderColor' => $cityColor[$parts[0]],
          'fill' => false,
          'hidden' => true,
          'data' => array(),
        );
      }
      $datasets[$targetKey][$cityKey]['data'][] = round($result[$targetKey][$k]['sum'][$cityKey] / $result[$targetKey][$k]['count'][$cityKey], 2);
    }
  }
}
